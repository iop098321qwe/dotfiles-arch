    ...
    BorderSurface {
      ...
      Item {
        ...
        Keys.onPressed: function(event) {
          // Local Ctrl+h/j/k/l navigation aliases.
          var controlOnly = event.modifiers === Qt.ControlModifier

          if (root.deleteConfirmOpen) {
            if (controlOnly && (event.key === Qt.Key_H || event.key === Qt.Key_L)) {
              deleteConfirm.selectedIndex = deleteConfirm.selectedIndex === 0 ? 1 : 0
              event.accepted = true
            } else if (deleteConfirm.handleKey(event)) {
              event.accepted = true
            }
            return
          }
          ...
          if (event.key === Qt.Key_Delete) {
            ...
            event.accepted = true
              ...
              && (event.key === Qt.Key_Backspace || event.key === Qt.Key_Left
                  || (controlOnly && event.key === Qt.Key_H))) {
            root.goBack()
            event.accepted = true
          } else if (event.key === Qt.Key_Up || (controlOnly && event.key === Qt.Key_K)) {
            root.select(-1)
            event.accepted = true
          } else if (event.key === Qt.Key_Down || (controlOnly && event.key === Qt.Key_J)) {
            root.select(1)
            event.accepted = true
            ...
            event.accepted = true
              ...
              || event.key === Qt.Key_Right || (controlOnly && event.key === Qt.Key_L)) {
            if (root.dmenuActive) {
